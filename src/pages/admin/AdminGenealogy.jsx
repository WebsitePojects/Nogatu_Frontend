  ...
  return (
    ...
    >
      ...
      <div className="mt-5 rounded-xl px-3 py-2 text-[11px] font-semibold inline-flex items-center gap-2"
        ...
        Open slot on this branch
      </div>

      <p className="mt-4 text-[11px]" style={{ color: 'rgba(255,255,255,0.45)' }}>
        {isDarkMode
          ? 'Admin can inspect the left and right opening positions here while the tree keeps the same branch logic as the member view.'
          : 'Admin can inspect the left and right opening positions here while the tree keeps the same branch logic as the member view.'}
      </p>
    </button>
  );
      ...
      try {
        ...
        if (!cancelled) {
          ...
          if (treeRes.data.rootUid && String(treeRes.data.rootUid) !== String(rootId)) {
            const nextParams = { id: String(treeRes.data.rootUid), depth: String(selectedDepth) };
    ...
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p className="text-sm mt-3" style={{ color: 'rgba(255,255,255,0.55)' }}>
            This mirrors the member-side tree canvas, but admin can inspect deeper binary levels across the wider network.
          </p>
        </div>
        ...
        <form onSubmit={handleSearchSubmit} className="flex flex-col sm:flex-row gap-2 sm:items-center">
          <input
            ...
            className="glass-input rounded-xl px-4 py-2.5 text-sm min-w-[260px]"
            placeholder="Type a username to open its tree"
          />
          <button
            type="submit"
            className="gold-btn rounded-xl py-2.5 px-4 text-sm"
          >
            Open Tree
          </button>
          <button
      ...
      <div className="glass-card relative z-30 rounded-2xl p-5">
        ...
        {loading && loadProgress > 0 && (
          <div className="mt-4">
            <div className="flex items-center justify-between text-[11px] mb-2" style={{ color: 'rgba(255,255,255,0.56)' }}>
      ...
      {!loading && !tree ? (
        <div className="glass-card rounded-2xl p-10 text-center">
          ...
          <h2 className="font-display text-xl font-semibold text-white">{rootId || rootUsername ? 'Genealogy could not be loaded' : 'Open an account tree'}</h2>
            ...
            <div className="glass-card rounded-2xl p-5">
              ...
              <div className="grid grid-cols-2 gap-3 mt-4 text-sm">
                <div className="rounded-xl p-3" style={{ background: 'rgba(255,255,255,0.04)' }}>
                  ...
                  <p className="font-bold text-white mt-1">{tree?.username || rootUsername || 'Member'}</p>
                </div>
                <div className="rounded-xl p-3" style={{ background: 'rgba(255,255,255,0.04)' }}>
